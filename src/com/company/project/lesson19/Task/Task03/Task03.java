...
public class Task03 {
    ...
    public static Double task01(ArrayList<Article> articles) {
        ...
        return /*Double avgAge =*/ articles.stream()
                .flatMapToInt(article -> article.getAuthors().values()
                        .stream()
                        .mapToInt(author -> {
    ...
    public static List<String> task02(ArrayList<Article> articles, Article.Category category) {
        ...
        return /*List<String> emails =*/ articles.stream()
                .filter(article -> article.getCategory() == category)
                .flatMap(article -> article.getAuthors().values()
                        .stream().map(author -> author.getEmail())).toList();
